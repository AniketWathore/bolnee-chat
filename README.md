# Bolnee — Multi-tenant RAG Chatbot Platform

AI chatbots grounded in your own knowledge (websites, PDFs, docs) via server-side retrieval. Customers create a bot, add sources, configure provider, and embed a snippet.
...
# Optional: set LLM_BASE_URL / LLM_API_KEY / LLM_MODEL for global fallback
npm install
npm run lint   # tsc --noEmit
npm run build  # vite + esbuild
npm run dev    # starts on http://localhost:3000 (auto-fallback to 3001 if busy)
```

Provider setup per chatbot (dashboard Step 3 or API):
- OpenAI: baseUrl https://api.openai.com/v1 + API key
- Ollama (keyless): baseUrl http://localhost:11434/v1, keep key empty
- vLLM / LM Studio / other self-hosted: set baseUrl + model + key if required

Dashboard flow: Create chatbot (name + optional avatar) → Add knowledge (URL and/or files; URL-only/files-only/both) → Choose provider/model → Processing (poll source status) → Copy embed snippet.

Embed snippet (never contains API keys):
...
## Server-side RAG (Current Direction)

The browser-only architecture above is retained as a legacy fallback. New
embeds should use the server-side `chatUrl` generated by the dashboard:

```text
website/PDF -> source ingestion -> chunks -> SQLite index
visitor     -> server retrieval -> OpenAI-compatible model -> SSE response
```
...
Set `LLM_BASE_URL`, `LLM_MODEL`, and optionally `LLM_API_KEY` in `.env`. Hosted
providers and local OpenAI-compatible gateways such as Ollama and vLLM are
supported. The local SQLite database is created at `data/bolnee.db` and is
ignored by git.

Authenticated users can add website URLs and PDF/text files from the API:

```bash
curl -X POST http://localhost:3000/api/knowledge/sources/BOT_ID \
  -H "Authorization: Bearer TOKEN" \
  -F "url=https://example.com"
```

See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) for the migration plan toward
PostgreSQL, `pgvector`, object storage, background workers, and provider
registries.
