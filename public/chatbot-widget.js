  ...
  function doSend() {
    ...
    if (intentDetector && knowledgeData) {
      try {
        ...
        // Knowledge intent → generate response via model with data block
        if (detection.confidence >= 25 && KNOWLEDGE_INTENTS[detection.intent]) {
          var pd = detection.promptData;
          if (pd && pd.dataBlock) {
            addDebug('[Response] Model with knowledge data');
            var prefix = 'Based on your query, ';
            var modelMessages = [
              { role: 'system', content: 'You are a concise assistant. Answer in 1-2 sentences using only the data below. Never make up information.' },
              { role: 'user', content: pd.dataBlock },
              { role: 'assistant', content: prefix }
            ];
            streamEl = null;
            rmTyping();
            streamEl = addMsg('bot', prefix);
            inp.disabled = true; send.disabled = true;
            worker.postMessage({ type: 'GEN', messages: buildQwenPrompt(modelMessages), config: { max_new_tokens: 40, temperature: 0.5 } });
            handled = true;
          }
        }
      } catch (e) {
