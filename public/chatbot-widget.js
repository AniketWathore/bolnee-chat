(function () {
  ...
  var THEME        = (cfg.theme || 'dark').toString().toLowerCase();
  // Fetch actual theme from server if not explicitly set in embed (covers old embeds)
  // We default to dark so stale white widget becomes dark even without regenerated embed
  ...
  document.head.insertAdjacentHTML('beforeend', '<style>' +
    ...
    '#_cw_ms{flex:1;overflow-y:auto;padding:14px;display:flex;flex-direction:column;gap:10px;scroll-behavior:smooth;background:' + C.winBg + '}' +
    '#_cw_ms::-webkit-scrollbar{width:3px}' +
    '#_cw_ms::-webkit-scrollbar-thumb{background:' + C.scrollThumb + ';border-radius:99px}' +
    ...
    '._m.b ._mb{background:' + C.botBg + ';color:' + C.botText + ';border-bottom-left-radius:3px}' +
    '._m.u ._mb{background:' + ACCENT + ';color:#fff;border-bottom-right-radius:3px}' +
    '._ml{font-size:10.5px;color:' + C.label + ';margin-bottom:3px;font-weight:500}' +
    '._m.u ._ml{text-align:right}' +
    '._dots{display:inline-flex;gap:4px;padding:4px 0}' +
    '._dots span{width:6px;height:6px;border-radius:50%;background:' + C.dotsBg + ';animation:_dt 1.1s infinite ease-in-out}' +
    ...
    '#_cw_ia{display:flex;align-items:flex-end;gap:8px;padding:10px 12px;border-top:1px solid ' + C.iaBorder + ';flex-shrink:0;background:' + C.iaBg + '}' +
    '#_cw_i{flex:1;border:1.5px solid ' + C.inputBorder + ';border-radius:10px;padding:8px 12px;font-size:13.5px;resize:none;outline:none;max-height:90px;overflow-y:auto;line-height:1.5;background:' + C.inputBg + ';color:' + C.inputText + ';transition:border-color .2s;font-family:inherit}' +
    '#_cw_i:focus{border-color:' + ACCENT + ';background:' + C.inputBg + '}' +
    '#_cw_i:disabled{opacity:.45}' +
    '#_cw_i::placeholder{color:' + C.placeholder + '}' +
